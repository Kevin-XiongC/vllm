...
class FakeMoeAllToAll:
    def __init__(self):
        ...
        self.dispatched_payloads: tuple[torch.Tensor | None, ...] | None = None

    def dispatch(
        self,
        token_selected_experts: torch.Tensor,
        input_payloads: tuple[torch.Tensor | None, ...],
        **kwargs: Any,
...
def test_one_sided_prepare_uses_out_of_range_invalid_expert_id_without_expert_map(
    ...
    initialize_kwargs = manager.initialize_kwargs
    dispatch_kwargs = manager.moe_alltoall.dispatch_kwargs
    dispatched_payloads = manager.moe_alltoall.dispatched_payloads
    assert initialize_kwargs is not None
    assert dispatch_kwargs is not None
    assert dispatched_payloads is not None
    assert initialize_kwargs["dispatch_dtype_bytes_per_elem"] == 2
    assert initialize_kwargs["dispatch_scale_bytes_per_token"] == 0
    assert dispatch_kwargs["invalid_token_expert_id"] == 4
    assert dispatch_kwargs["expert_id_payload_index"] == 1
    assert dispatched_payloads[0] is a1
    assert dispatched_payloads[1] is topk_ids
    assert dispatched_payloads[2] is topk_weights
...
def test_one_sided_prepare_uses_nonlocal_padding_expert_with_expert_map(monkeypatch):
    ...
    assert dispatch_kwargs is not None
    assert dispatched_payloads is not None
    assert dispatch_kwargs["invalid_token_expert_id"] == 2
    assert dispatch_kwargs["expert_id_payload_index"] == 2
    assert dispatched_payloads[0] is a1q
    assert dispatched_payloads[1] is a1q_scale
    assert dispatched_payloads[2] is topk_ids
    assert dispatched_payloads[3] is topk_weights
