...
def test_novita_fused_rope_fp8_kvstore_matches_reference(
    ...
    # q/k go through rope: fused FMA vs. sequential reference may round to
    # adjacent FP8 buckets near quantization boundaries.
    # Bound is 1 ULP for e4m3: atol covers subnormals (2^-9), rtol covers
    # normals (max ULP/value ratio is 2^-3 = 0.125).
    torch.testing.assert_close(q_test_f, q_ref_f, atol=0.002, rtol=0.125)
    torch.testing.assert_close(k_test_f, k_ref_f, atol=0.002, rtol=0.125)
    # v is quantize+store only — must be bit-exact
    assert torch.equal(v_test_f, v_ref_f), "v_cache mismatch: expected bit-exact"
